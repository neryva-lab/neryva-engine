...
- **Legacy `src/modules/agent-studio`** is product furniture (`studio_project_keys` binding only, `drizzle/0006`). It is **not** the new `assistants`/`conversations`/`runs` domain. Do not add new conversation/run logic there — create `src/modules/assistants/` and `src/modules/conversations/` per ledger Phase 3–4. Rename is tracked as ledger 3.7.
