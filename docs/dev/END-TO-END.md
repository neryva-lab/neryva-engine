...
> **⚠️ ADR-005 & ADR-006 binding model:** The engine core is TypeScript on NestJS (`engine/src/`). The inventory and auth map below form **the complete behavioral specification** for the core logic (Identity, Tenancy, Orgs, Entitlements, Metering, Governance/Policy Decision, Audit, Corporate). Heavy runtime workloads are **Capability Deployments (Satellites)** connecting via the 4-part contract (ADR-006 D2) — the existing Python runtime is the first satellite (`agent-runtime`), serving `/v1/**` and `/surfaces/**` as an independent deployable (connect, don't port). The TS execution steps (M1–M8), engine layout, route namespaces, and the ingestion playbook live in [`modularity/plan.md`](modularity/plan.md) — read it alongside the E-phases below: E1–E5 correspond to M3–M5 (net-new TS modules), E6/E8 to M6, and the satellite handover to M7. The Python runtime serves production as a connected capability deployment.
