...
| Contract | `products/agent-studio/contracts/agent-definition/v1.schema.json` | `AgentDefinition v1`: required `{agent_id, version, instructions, model_policy, context_policy, tools, guardrails}`; `agent_id` kebab `[a-z0-9-]+` 3..64; `tools[].approval ∈ {required,none}`; `guardrails.input_policy ∈ {default,strict,permissive}`, `output_policy ∈ {brand-safe,default,strict}`; optional `budget_policy` + `retrieval_policy`. Engine stays source of truth for persisted versions; consumers merge partial definitions over documented defaults (reference implementation: `parseDefinition()` at `console/neryva-website/src/hooks/studio/useAgentAuthoring.ts:123-195`) |
...
2. **G2 — Create drops the definition.** Engine `POST .../assistants` accepts name/description only, so no client can create a fully-specified agent in one governed step; the definition only lands via a separate unlinked `POST .../versions` call.
...
| `tools` (Engine entries) | `search_knowledge(access=read)`, `create_case(access=write, approval=required)`, `escalate_handoff(access=write, approval=optional)` — handoff is always-available by policy, not by a `never` value (no such Engine value exists) | `search_memory(access=read)`, `schedule_meeting(access=write, approval=required)`, `send_email(access=write, approval=required)` + catalog `REQUIRED` |
...
|---|---|---|---|---|
| `READ_ONLY` | `search_knowledge`, `search_memory`, `get_order`, `summarize_logs` | `access=read, approval=optional` (console `never`) | `effect_class=READ_ONLY`, `approval_requirement=NONE`, `idempotent` annotation true | none |
...
└── engine/drizzle/00XX_assistant_templates.sql  # NEW — seed + install records (see §7.2)
```
...
4. **Extend `POST /console/org/:orgId/assistants`** to accept optional `template?: {slug, version?}` OR `definition?: AssistantPayload` (validated by `validateAssistantPayload`, secrets rejected; template-only extensions in §4.2 are stripped with a 422 listing them). Keeps back-compat (name-only create still works).
...
- [ ] Tool pins resolve to ENABLED catalog rows at install org (else `available: false` + reasons — never silent).
- [ ] `allowed_models` ⊆ org `model_catalog` ENABLED set (existing `rejectUnknownModels`) + residency coverage.
...
1. **PR-A (schema + seed, ledger 3.1 ext):** migration `00XX` (registry + installs) + `ownership-map.json` + `TemplatesService.list/get` + `GET .../assistant-templates` with compatibility reasons. No consumer change. Gate: RLS tests for `assistant_installs`; seed hash parity test.
...
3. **PR-C (12 Tier-1 template contents):** `products/agent-studio/templates/*` BOMs (+ `release_policy.yaml`, `evaluators.yaml`) + `registry.json` + lint CI. Gate: all 12 pass §9 template-CI (Studio validator + Engine bounds + compile-check); reviewers sign instructions tone per template.
