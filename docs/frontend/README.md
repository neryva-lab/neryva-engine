...
5. **`sso.md`** — pure-SSO sign-in plan (Google/GitHub federated through the Engine OP): why no vendors, the five-step flow mapped to code, app-registration steps with exact redirect URIs, engine + console work items, acceptance gate. Blocked on IdP credentials (Phase A).
