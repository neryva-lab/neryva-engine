/**
 * PostgreSQL escalation repository (P3) — human handoff / live-agent takeover
 * (FL-1.7). Mechanical move of the `EscalationsService` transaction bodies.
 *
 * Every mutation is one transaction: the escalation row AND the conversation
 * status flip AND the outbox event commit or roll back together (invariant 7).
 * The queue list is the ordered wait surface for agent consoles.
 *
 ...
 *
 * Validation (reason/text/agent) stays in the service; audit replay is
 * driven by the `created` / `transitioned` flags (the service holds every
 * audit field, so no audit trail crosses this interface).
 */
...
export class PgEscalationRepository implements IEscalationRepository {
  ...
  async escalate(input: {
    ...
    return this.db.withOrg(input.orgId, async (tx) => {
      ...
      const open = await tx
        ...
        .limit(1);
      if (open.length > 0) {
        return { escalation: open[0], created: false };
      }
      ...
      return { escalation: inserted[0], created: true };
    });
  }
    ...
    return this.db.withOrg(orgId, async (tx) => {
      ...
      if (escalation.state === 'CLAIMED') {
        if (escalation.claimedBy === agent) {
          return { escalation, transitioned: false }; // replay
        }
      ...
      return { escalation: updated[0], transitioned: true };
    });
  }
