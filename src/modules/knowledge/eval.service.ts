...
} from '../assistants/repositories/repository-tokens';
import type { IEvalDatasetRepository } from './repositories/eval-dataset.repository';
import type { IEvalRunRepository } from './repositories/eval-run.repository';
import type { IAssistantRepository } from '../assistants/repositories/assistant.repository';
import type { IAssistantVersionRepository } from '../assistants/repositories/assistant-version.repository';
import type { IPolicySnapshotRepository } from '../assistants/repositories/policy-snapshot.repository';
import type { ITemplateRepository } from '../assistants/repositories/template.repository';
import type { IToolCatalogRepository } from '../assistants/repositories/tool-catalog.repository';
import type { NewEvalCase } from './repositories/repository-types';
...
interface ResolvedTemplatePolicy {
  slug: string;
  version: string;
...
export class EvalService {
  ...
  constructor(
    @Inject(EVAL_DATASET_REPOSITORY) private readonly datasets: IEvalDatasetRepository,
    @Inject(EVAL_RUN_REPOSITORY) private readonly runs: IEvalRunRepository,
    ...
    @Inject(TOOL_CATALOG_REPOSITORY) private readonly toolCatalog: IToolCatalogRepository,
  ...
  async createDataset(input: {
    ...
    const id = uuidv7();
    const row = await this.datasets.create(input.orgId, {
      id,
  ...
  async addCases(input: {
    ...
    const drafts: NewEvalCase[] = parsed.map((c) => ({
      id: uuidv7(),
      organizationId: input.orgId,
    ...
    const added = await this.datasets.appendCases(input.orgId, input.datasetId, drafts);
    return { added };
  }

  async listDatasets(orgId: string): Promise<unknown[]> {
    assertUuid(orgId, 'orgId');
    return this.datasets.list(orgId, 100);
  }
  ...
  async listCases(input: {
    ...
    const ds = await this.datasets.findById(input.orgId, input.datasetId);
    if (!ds) {
      throw ApiError.notFound('eval dataset');
    }
    const { cases, total } = await this.datasets.listCases(input.orgId, input.datasetId, {
      limit,
      offset,
    });
    return {
  ...
  async updateCase(input: {
    ...
    const c = result.data;
    const r = await this.datasets.updateCase(input.orgId, input.datasetId, input.caseId, {
      input: c.input,
      expected: c.expected,
      rubric: c.rubric ?? null,
    });
  ...
  async deleteCase(input: {
    ...
    assertUuid(input.datasetId, 'datasetId');
    assertUuid(input.caseId, 'caseId');
    const deleted = await this.datasets.deleteCase(input.orgId, input.datasetId, input.caseId);
    if (!deleted) {
      throw ApiError.notFound('eval case');
    }
    await this.audit.add({
  ...
  async deleteDataset(input: { orgId: string; datasetId: string; actor: string }): Promise<{ deleted: true }> {
    ...
    let outcome: 'deleted' | 'not_found' | 'has_runs';
    try {
      outcome = await this.datasets.deleteIfNoRuns(input.orgId, input.datasetId);
    } catch (err) {
      // Best-effort under READ COMMITTED: a run inserted between the guard
      // read and the delete loses to the FK — map that to `has_runs` per
      // the repository contract (legacy would have surfaced a raw 500).
    ...
    if (outcome === 'has_runs') {
      const runCount = await this.runs.countRunsForDataset(input.orgId, input.datasetId);
  ...
  async exportDataset(input: {
    ...
    assertUuid(input.orgId, 'orgId');
    assertUuid(input.datasetId, 'datasetId');
    const exported = await this.datasets.exportAll(input.orgId, input.datasetId);
    if (!exported) {
      throw ApiError.notFound('eval dataset');
    }
    const { dataset: ds, cases: rows } = exported;
    const slug = ds.name.replace(/[^a-zA-Z0-9-_]+/g, '-').slice(0, 64) || 'dataset';
    if (input.format === 'csv') {
  ...
  async importDataset(input: {
    ...
    await this.audit.add({
      ...
      actorId: input.actor,
      tenantId: input.orgId,
      details: { format: input.format, added },
    });
    return { added };
  }
  ...
  async promoteCandidateCase(input: {
    ...
    assertUuid(input.datasetId, 'datasetId');
    assertUuid(input.caseId, 'caseId');
    const sourceDataset = await this.datasets.findById(input.orgId, input.datasetId);
    if (!sourceDataset) {
      throw ApiError.notFound('eval dataset');
    }
    ...
    const target = await this.datasets.findByName(input.orgId, targetName);
    if (!target) {
      throw ApiError.conflict('candidate target dataset does not exist', { target: targetName });
    }
    const promoted = await this.datasets.promoteCandidate(
      input.orgId,
      input.datasetId,
      target.id,
      input.caseId,
    ...
    await this.audit.add({
      ...
      details: {
        ...
        promoted_case_id: promoted.promotedCaseId,
      },
    });
    return { promoted_case_id: promoted.promotedCaseId };
  }
  ...
  async rejectCandidateCase(input: {
    ...
    if (!targetDatasetName(sourceDataset.name)) {
      throw ApiError.validation({
  ...
  async startRun(input: {
    ...
    const version = await this.versions.getVersion(input.orgId, input.assistantVersionId);
    ...
    if (!version || !isEvaluableVersionStatus(version.status)) {
      throw ApiError.validation({
    ...
    const dataset = await this.datasets.findById(input.orgId, input.datasetId);
    if (!dataset) {
      throw ApiError.notFound('eval dataset');
    }
    // W2.4 (drizzle/0070) — authoritative content pin: the version's CURRENT
    // snapshot, so the eval is forever bound to the content that was live at
    // start time. Fail closed when the version has no snapshot (drafts
    // without one cannot run — the executor's snapshot-required join would
    // refuse every case anyway).
    //
    // P3 SEAM (documented): the legacy code resolved the snapshot in the
    // SAME transaction that created the run. The ports split the read from
    // the co-commit — a concurrent draft edit landing between the two reads
    // pins the pre-edit snapshot row instead. The pin stays immutable and
    // content-addressed (the old row is never mutated), so the run still
    // executes exactly one consistent content; the publish gate verifies
    // pin hash == executed hashes regardless.
    const snapshot = await this.snapshots.getSnapshotForVersion(
      input.orgId,
      version.assistantId,
      input.assistantVersionId,
    );
    if (!snapshot) {
      throw ApiError.conflict('assistant version has no policy snapshot');
    }
    const id = uuidv7();
    return this.runs.createWithOutboxEvent(
      input.orgId,
      {
        id,
        organizationId: input.orgId,
  ...
  async detectModelDrift(
    ...
    // Tenant-scoped read (RLS / explicit org predicate): a foreign-org id yields no row.
    if (!assistant) {
    ...
    const snapshot = await this.snapshots.getSnapshotForVersion(
      orgId,
      assistantId,
      assistant.activeVersionId,
    );
    const models =
      ((snapshot?.modelRef ?? {}) as { models?: Array<Record<string, unknown>> }).models ?? [];
  ...
  async startShadowEval(input: {
    ...
    assertUuid(input.orgId, 'orgId');
    if (await this.runs.hasRecentShadowRun(input.orgId, input.versionId, 24)) {
  ...
  private async resolveTemplateDataset(orgId: string, assistantId: string): Promise<string | null> {
    ...
    const name = `template:${install.slug}@${install.templateVersion}`;
    const dataset = await this.datasets.findByName(orgId, name);
    return dataset?.id ?? null;
  }

  async listRuns(orgId: string, datasetId?: string): Promise<unknown[]> {
    assertUuid(orgId, 'orgId');
    return this.runs.list(orgId, { datasetId, limit: 100 });
  }
  ...
  async completeRun(input: {
    ...
    // P3 SEAM (documented): the legacy code ran every read below plus the
    // terminal CAS in ONE transaction. The ports split the decision reads
    // from the CAS — `completeIfOpen` keeps the `state IN
    // ('pending','running')` fence as the correctness-critical gate (a
    // duplicate delivery still returns null → 409, never a double
    // completion), but the decision inputs are read just before the fence
    // rather than inside the same unit of work.
    const run = await this.runs.findById(input.orgId, input.evalRunId);
    if (!run) {
    ...
    const version = await this.versions.getVersion(input.orgId, run.assistantVersionId);
    if (!version) {
      throw ApiError.conflict('eval run references a missing assistant version');
    }
    const dataset = await this.datasets.findById(input.orgId, run.datasetId);

    // Template linkage: seeded datasets are named template:<slug>@<version>.
    const template = await this.resolveTemplatePolicy(input.orgId, version.assistantId);
    ...
    engineChecks.set(
      ...
      await this.verifyToolPinsLive(input.orgId, version.toolPolicy),
    );
    ...
    if (typeof regressionBound === 'number') {
      ...
      // version succeeded.
      const published = (await this.versions.listVersions(input.orgId, version.assistantId))
      ...
      const previous = currentPublished
        ? (published.find((v) =>
            version.version === 0
              ? v.version <= currentPublished.version
              : v.version < version.version,
          ) ?? null)
        : null;
      if (previous) {
        const prevScore = await this.runs.latestCompletedRunScore(
          input.orgId,
          previous.id,
          run.datasetId,
        );
        if (prevScore !== null) {
          const delta = Number(prevScore) - score;
          if (delta > regressionBound) {
            blockReasons.push(
              `regression ${delta.toFixed(4)} exceeds bound ${regressionBound} (previous score ${prevScore})`,
            );
          }
        }
      }
    ...
    if (run.policySnapshotId) {
      const pin = await this.snapshots.getSnapshot(input.orgId, run.policySnapshotId);
      pinHash = pin?.hash ?? null;
    }
    const executedHashes = await this.runs.listExecutedContentHashes(input.orgId, input.evalRunId);
    const distinctHashes = [...new Set(executedHashes)];
    ...
    const provenance = await this.assembleProvenance(input.orgId, {
      run,
      version,
      dataset,
      template,
    ...
    const completed = await this.runs.completeIfOpen(input.orgId, input.evalRunId, {
      state: 'completed',
      results: parsed,
  ...
  private async resolveTemplatePolicy(
    orgId: string,
    assistantId: string,
  ): Promise<ResolvedTemplatePolicy | null> {
    const install = await this.templates.getInstallByAssistantId(orgId, assistantId);
    if (!install || install.organizationId !== orgId) {
      return null;
    }
    const template = await this.templates.getTemplateBySlugAndVersion(
      install.slug,
      install.templateVersion,
    );
    if (!template) {
      return {
        slug: install.slug,
  ...
  private async verifyToolPinsLive(orgId: string, toolPolicy: unknown): Promise<boolean> {
    const tools =
    ...
    for (const name of names) {
      const row = await this.toolCatalog.getTool(orgId, name);
      if (!row || !row.enabled) {
        return false;
      }
  ...
  private async hashDatasetCases(orgId: string, datasetId: string): Promise<string> {
    const cases = await this.runs.listCasesForHash(orgId, datasetId);
  ...
  private async assembleProvenance(
    orgId: string,
    input: {
      run: { id: string; datasetId: string; attemptsPerCase: number; policySnapshotId: string | null };
    ...
    const catalogRows = await this.toolCatalog.listTools(orgId, { includeDisabled: true });
    ...
    //
    // P3 NOTE: the legacy code looked this row up by (assistantVersionId,
    // hash). `evaluatedContentHash` is only ever the pin row's own hash
    // (§3b sets it solely when the executions' distinct hash == the pin
    // hash), so resolving by the run's `policySnapshotId` returns the same
    // row — without a dedicated by-hash port.
    const snapshot =
      input.evaluatedContentHash && input.run.policySnapshotId
        ? await this.snapshots.getSnapshot(orgId, input.run.policySnapshotId)
        : null;
    ...
    return {
      ...
      dataset: input.dataset
        ? {
            ...
            content_hash: await this.hashDatasetCases(orgId, input.dataset.id),
          }
        : null,
  ...
  async evaluateRetrieval(input: {
    ...
    // No dataset-exists 404 here (legacy behavior): an unknown dataset
    // yields an empty case list and the zero aggregate below.
    const { cases } = await this.datasets.listCases(input.orgId, input.datasetId, {
      limit: 100,
      offset: 0,
    });
    const scored: Array<{
