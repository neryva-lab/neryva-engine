...
export class OrgAuditService {
  ...
  /** Distinct values for the filter dropdowns (bounded, cached by the caller's HTTP layer). */
  async filterFacets(orgId: string): Promise<{ actions: string[]; resourceTypes: string[] }> {
    const actions = await this.db.root.execute<{ action: string }>(sql`
      select distinct action from audit_events where tenant_id = ${orgId} order by action limit 500
    `);
